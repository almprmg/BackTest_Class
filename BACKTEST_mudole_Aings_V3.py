...
if __name__ == '__main__':
        ...
        #for i in fi:
                ...
                df=pd.read_csv(f"C:\\Users\\dell\\Desktop\\مجلد جديد\\data\\5m\\AGLDUSDT.csv")
                ...
                back = BACKTESTING(data=df1,Asign=Asign,TP=TP,SL=SL,symbol=i[:-4])        
                back.start(TR=True,Asign=True)
                tiker.append(i)
                li.append(back.Result.Cash.tail(1).values)
                x = pd.DataFrame(li,columns=['Cash'])
                if len(back.Enter)-len(back.TS_A) == 0:

                    cast.append(back.Result.Cash_A.tail(1).values)
                ...
                #print(li
                #result_T_20_3_A1.5.0.csv على اساس ان الهدف الاول 20%
                x.insert(loc=0 ,column='symbol', value=tiker)
                x.to_csv(f'new/result_T_T{TP}_SL{SL}_A{Asign}.0.csv')
                #print(x)
